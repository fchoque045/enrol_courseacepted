...
class enrol_courseacepted_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/courseacepted:config', $context)) {
            return null;
        }
        return new moodle_url('/enrol/courseacepted/edit.php', array('courseid' => $courseid));
    }

    // check if enrollment is enabled
    public function allow_apply(stdClass $instance) {
        if ($instance->status != ENROL_INSTANCE_ENABLED) {
            return get_string('cantenrol', 'enrol_courseacepted');
        }
        if (!$instance->customint6) {
            // New enrols not allowed.
            return get_string('cantnewenrol', 'enrol_courseacepted');
        }
        return true;
    }
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('enrol/courseacepted:config', $context);
    }
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if ($instance->enrol !== 'courseacepted') {
        ...
        if (has_capability('enrol/courseacepted:config', $context)) {
            $editlink = new moodle_url("/enrol/courseacepted/edit.php", array('courseid' => $instance->courseid, 'id' => $instance->id));
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/courseacepted/classes/form/courseacepted_form.php");
        ...
        $mform = new enrol_courseacepted_form(null, $instance);
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol_user($instance, $ue) && has_capability("enrol/courseacepted:unenrol", $context)) {
        ...
        if ($this->allow_manage($instance) && has_capability("enrol/courseacepted:manage", $context)) {
    ...
    public function enrol_self(stdClass $instance, $data = null) {
        ...
        //$this->confirm_enrolment(array($instance->id));
...
/**
    ...
    //         $instance = $DB->get_record('enrol', array('id' => $userenrolment->enrolid, 'enrol' => 'courseacepted'), '*', MUST_EXIST);

    //         // Check privileges.
    //         $context = context_course::instance($instance->courseid, MUST_EXIST);
    //         if (!has_capability('enrol/courseacepted:manageapplications', $context)) {
    //             continue;
    //         }
    ...
    //     global $CFG;
    //     require_once($CFG->dirroot.'/enrol/courseacepted/notification.php');
